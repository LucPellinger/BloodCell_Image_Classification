...
def compile_and_fit(model,
    ...
    if early_stopping:
      callbacks.append(tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True, verbose=1))
      callbacks.append(tf.keras.callbacks.ModelCheckpoint(filepath='best_model.h5',  # File path where the model will be saved
                                       monitor='val_loss',        # Metric to monitor
                                       mode='min',                # We want to minimize the validation loss
                                       save_best_only=True,       # Save only the best model
                                       verbose=1))
    
    # ✅ Enable TensorBoard graph tracing
    ...
    # ✅ Export trace
    tf.summary.trace_export(name="model_trace", step=0, profiler_outdir=name_tensorboard)
